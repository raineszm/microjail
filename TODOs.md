...
## 1. Endpoint capability setup UX [done]

Implemented in change `endpoint-capability-cli`. CLI commands `microjail cap add endpoint`
and `microjail cap remove endpoint` with state-aware save/apply, validation, idempotent
adds, `--replace` for changes, and doc fixes.
...
## 2. Batched refreshes [done]

Implemented in change `batched-refreshes`. `Workshop.batch()` context manager
defers `workshop refresh` + tunnel connect across N endpoint capabilities to
a single flush. YAML writes are immediate; only refresh+connect is deferred.
...
- ✅ Preserve stateless safety: no broad cached lock state.
