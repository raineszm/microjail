...
`MicroJail.pre_launch_verify()` runs all Gate and Capability verifiers after lockdown application but before any workload process is created. It collects non-fatal failures, stops on fatal failures, and surfaces warnings and unsupported notes to the CLI. This ensures the environment is ready for the workload before it starts.
