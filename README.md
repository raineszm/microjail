...
1. **Provision** — Install packages and set up the workspace through thin wrappers (`apt`, `uv`, and so on). Everything lands in the container or the mounted workspace.
1. **`run`** — Lock the environment and execute the workload. Before the process starts, `microjail` runs a set of checks (see [gates](#lock-gates)); the command only spawns if they all pass.
...
- **Config is readonly** --- the agent cannot modify its own jail.
